...
type HotkeyErrors = Partial<Record<keyof HotkeySettings | "configuration", string>>;
...
type CrosshairSettings = Omit<Settings, "hotkeys" | "hotkeyErrors">;
type CrosshairKey = keyof CrosshairSettings;
...
const icon = (name: string) => {
  const paths: Record<string, string> = {
    crosshair: '<circle cx="12" cy="12" r="5"/><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/>',
...
const range = (key: CrosshairKey, label: string, min: number, max: number, suffix = "") => `
...
function hotkeyRow(key: keyof HotkeySettings, label: string, description: string, defaultValue: string) {
...
function renderShell() {
  document.querySelector<HTMLDivElement>("#app")!.innerHTML = `
    <main class="window-shell">
      ...
      <footer>
        <div class="save-state"><i></i><span>Changes save automatically</span></div>
...
function bindEvents() {
  ...
  document.querySelector("#minimize")?.addEventListener("click", () => invoke("minimize_settings"));
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-hotkey]").forEach((button) => {
    button.addEventListener("click", () => beginRecording(button.dataset.hotkey as keyof HotkeySettings));
  });
  document.querySelector("#reset-hotkeys")?.addEventListener("click", resetHotkeys);
}
...
async function beginRecording(key: keyof HotkeySettings) {
  try {
    ...
    renderShell();
    document.querySelector<HTMLButtonElement>(`[data-hotkey="${key}"]`)?.focus();
  } catch (error) {
...
async function handleRecordingKeyDown(event: KeyboardEvent) {
  ...
  const otherKey: keyof HotkeySettings = key === "closeApp" ? "showSettings" : "closeApp";
  ...
  try {
    const accepted = await invoke<HotkeySettings>("update_hotkeys", { hotkeys: proposedSettings });
...
function showHotkeyError(error: unknown, rerender = true) {
  hotkeyStatus = typeof error === "string" ? error : error instanceof Error ? error.message : String(error);
  hotkeyStatusError = true;
...
function updateFromInput(input: HTMLInputElement) {
  ...
  document.querySelectorAll<HTMLInputElement>(`[data-key="${key}"]`).forEach((peer) => {
...
function applyPreset(preset: string) {
  const presets: Record<string, Partial<Settings>> = {
    classic: { length: 20, thickness: 2, gap: 6, centerDot: true, dotSize: 3, tStyle: false },
    compact: { length: 10, thickness: 3, gap: 3, centerDot: false, tStyle: false },
    dot: { length: 1, thickness: 1, gap: 0, centerDot: true, dotSize: 5, tStyle: false },
    open: { length: 16, thickness: 2, gap: 12, centerDot: false, tStyle: false },
    precision: { length: 28, thickness: 1, gap: 4, centerDot: true, dotSize: 2, tStyle: true },
  };
...
async function syncAndSave(rerender: boolean) {
  const crosshair = await invoke<CrosshairSettings>("update_settings", { settings });
  settings = { ...settings, ...crosshair };
  if (rerender) renderShell();
}
...
function drawPreview() {
  ...
  if (!settings.tStyle) arms.push([cx, cy - settings.gap - settings.length, cx, cy - settings.gap]);
  ...
  if (settings.centerDot) {
    ...
    ctx.fillRect(cx - settings.dotSize / 2, cy - settings.dotSize / 2, settings.dotSize, settings.dotSize);
  }
}
...
async function boot() {
  ...
  window.addEventListener("beforeunload", () => {
    if (nativeRecording) void invoke("set_hotkey_recording", { recording: false });
  });
}
